/* Skill-Lite — LangGraph Tutorial Navigation */

(function () {
  var SECTIONS = [
    { label: 'Getting Started', pages: [
      { file: 'introduction.html',  icon: '🔷', title: 'Introduction' },
      { file: 'state.html',         icon: '📦', title: 'State Management' },
      { file: 'nodes-edges.html',   icon: '🔗', title: 'Nodes & Edges' },
    ]},
    { label: 'Core Features', pages: [
      { file: 'checkpointing.html', icon: '💾', title: 'Checkpointing' },
      { file: 'multi-agent.html',   icon: '🤖', title: 'Multi-Agent Systems' },
    ]},
    { label: 'Advanced', pages: [
      { file: 'human-in-the-loop.html', icon: '🧑', title: 'Human-in-the-Loop' },
      { file: 'streaming.html',         icon: '⚡', title: 'Streaming & Async' },
    ]},
  ...
  function isTutorialPage() {
    ...
    return dir === 'langgraph' && PAGES.some(function (p) { return p.file === file; });
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ── Dropdown ── */
    var dropdown = document.getElementById('langgraphDropdown');
    if (dropdown) {
    ...
    if (side && onTutorial) {
      var sHtml = '<div class="side-title"><span class="langgraph-logo">🔷</span>LangGraph Tutorial</div>';
