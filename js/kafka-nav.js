/* Skill-Lite — Kafka Tutorial Navigation */
...
(function () {
  var SECTIONS = [
    ...
    { label: 'Core Concepts', pages: [
      { file: 'producers.html',       icon: '📤', title: 'Producers' },
      { file: 'consumers.html',       icon: '📥', title: 'Consumers & Consumer Groups' }
    ]},
    { label: 'Processing & Integration', pages: [
      { file: 'kafka-streams.html',   icon: '🌊', title: 'Kafka Streams' },
      { file: 'kafka-connect.html',   icon: '🔌', title: 'Kafka Connect' }
    ]},
    { label: 'Schema & Reference', pages: [
      { file: 'schema-registry.html', icon: '📋', title: 'Schema Registry & Avro' },
      { file: 'cheatsheet.html',      icon: '⌨️', title: 'CLI Cheat Sheet' }
    ]}
  ];
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ── 1. Top-menu "Kafka" dropdown ── */
    var dropdown = document.getElementById('kafkaDropdown');
    if (dropdown) {
    ...
    if (side && onTutorial) {
      var sHtml = '<div class="side-title"><span class="kafka-logo">📨</span>Kafka Tutorial</div>';
