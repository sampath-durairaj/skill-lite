/* Skill-Lite — LangChain Tutorial Navigation */

(function () {
  var SECTIONS = [
    { label: 'Getting Started', pages: [
      { file: 'introduction.html', icon: '🦜', title: 'Introduction' },
      { file: 'models.html',       icon: '🤖', title: 'Models & LLMs' },
      { file: 'prompts.html',      icon: '💬', title: 'Prompt Templates' },
    ]},
    { label: 'Building Blocks', pages: [
      { file: 'chains.html', icon: '⛓️', title: 'Chains & LCEL' },
      { file: 'memory.html', icon: '🧠', title: 'Memory' },
    ]},
    { label: 'Advanced', pages: [
      { file: 'agents.html', icon: '🕵️', title: 'Agents & Tools' },
      { file: 'rag.html',    icon: '📚', title: 'RAG & Vector Stores' },
    ]},
  ...
  function isTutorialPage() {
    ...
    return dir === 'langchain' && PAGES.some(function (p) { return p.file === file; });
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    /* ── Dropdown ── */
    var dropdown = document.getElementById('langchainDropdown');
    if (dropdown) {
    ...
    if (side && onTutorial) {
      var sHtml = '<div class="side-title"><span class="langchain-logo">🦜</span>LangChain Tutorial</div>';
